# Written 09/23/2021 by Jason David Chobirko

# This R script - LTR7_blockFilt.r - will be used alongside the bash script LTR7_muscleConsStrandFind.sh. It 
...
names <- c("LTR7B", "LTR7C", "LTR7bc", "LTR7o", "LTR7d1", "LTR7d2", "LTR7u2", "LTR7Y", "LTR7u1", "LTR7up2", "LTR7up1")
...
for (samp in 1:length(names)) {
  ...
  if (names[samp] == "LTR7o") {brkPt <- c(61, 110, 110, 154, 215, 302, sum(eval(parse(text = paste("align", names[samp], "seq", sep = "_")))[conRow, ] != "-") - 1) }
  if (names[samp] == "LTR7bc") {brkPt <- c(83, 132, 132, 176, 237, 324, sum(eval(parse(text = paste("align", names[samp], "seq", sep = "_")))[conRow, ] != "-") - 1) }
  if (names[samp] == "LTR7d1") {brkPt <- c(94, 145, 145, 199, 261, 348, sum(eval(parse(text = paste("align", names[samp], "seq", sep = "_")))[conRow, ] != "-") - 1) }
  if (names[samp] == "LTR7d2") {brkPt <- c(93, 144, 144, 198, 260, 347, sum(eval(parse(text = paste("align", names[samp], "seq", sep = "_")))[conRow, ] != "-") - 1) }
  if (names[samp] == "LTR7u2") {brkPt <- c(84, 136, 176, 220, 281, 368, sum(eval(parse(text = paste("align", names[samp], "seq", sep = "_")))[conRow, ] != "-") - 1) }
  if (names[samp] == "LTR7u1") {brkPt <- c(84, 136, 184, 239, 301, 388, sum(eval(parse(text = paste("align", names[samp], "seq", sep = "_")))[conRow, ] != "-") - 1) }
  if (names[samp] == "LTR7up2") {brkPt <- c(84, 136, 183, 238, 300, 387, sum(eval(parse(text = paste("align", names[samp], "seq", sep = "_")))[conRow, ] != "-") - 1) }
  if (names[samp] == "LTR7up1") {brkPt <- c(84, 136, 183, 238, 300, 387, sum(eval(parse(text = paste("align", names[samp], "seq", sep = "_")))[conRow, ] != "-") - 1) }
  ...
  while (count < brkPt[7]) {  # ncol(eval(parse(text = paste("align", names[samp], "seq", sep = "_"))))) {
    ...
    if (count %in% brkPt & eval(parse(text = paste("align", names[samp], "seq", sep = "_")))[conRow, curPos] != "-") { 
      ...
      # Use seqinr to write the .fa file but only if one vector has at least 1 element!
